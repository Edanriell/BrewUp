    ...
    : ServiceBase(loggerFactory), IAvailabilityQueryService
{
        ...
        Guid beerId, CancellationToken cancellationToken)
    {
        ...
        {
            var availability =
                await queries.GetByFilterAsync(a => a.BeerId.Equals(beerId.ToString()), 0, 20, cancellationToken);
            return availability.TotalRecords > 0
                ? new PagedResult<BeerAvailabilityJson>(availability.Results.Select(r => r.ToJson()), availability.Page,
                    availability.PageSize, availability.TotalRecords)

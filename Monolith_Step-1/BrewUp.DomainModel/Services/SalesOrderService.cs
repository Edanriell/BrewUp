    ...
    [FromKeyedServices("warehouse")] IRepository warehouseRepository) : ISalesOrderService
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            var availability =
                await warehouseRepository.GetByIdAsync<Availability>(row.BeerId.ToString(), cancellationToken);
            if (availability != null)
                beersAvailable.Add(row);
        }

        var aggregate = SalesOrder.CreateSalesOrder(salesOrderId, salesOrderNumber, orderDate, customerId, customerName,
            beersAvailable);

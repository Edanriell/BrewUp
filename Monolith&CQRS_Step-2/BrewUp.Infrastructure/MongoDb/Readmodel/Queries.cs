...
public abstract class Queries<T> : IQueries<T> where T : EntityBase
{
    ...
    protected Queries(IMongoClient mongoClient) { MongoClient = mongoClient; }
    ...
    public async Task<PagedResult<T>> GetByFilterAsync(
        Expression<Func<T, bool>>? query, int page, int pageSize, CancellationToken cancellationToken)
    {
        if (--page < 0)
            page = 0;

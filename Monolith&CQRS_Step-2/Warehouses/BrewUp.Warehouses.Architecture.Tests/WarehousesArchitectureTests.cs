...
public class WarehousesArchitectureTests
{
    ...
    {
        var warehousesModulePath =
            Path.Combine(VisualStudioProvider.TryGetSolutionDirectoryInfo().FullName, "Warehouses");
        var subFolders = Directory.GetDirectories(warehousesModulePath);

        var netVersion = Environment.Version;

        var warehousesAssemblies = (
            from folder in subFolders
